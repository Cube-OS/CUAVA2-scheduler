...
def main():
    ...
    pass_cuava_rc_path = "/home/cuava/Ops/restart_adcs_c2.txt"
    ...
    with open(gs_autorun_log_path, 'w') as gs_log:
        process_gsautorun = subprocess.Popen(
            [gs_autorun_path, "-i", pass_cuava_rc_path,"-t","8"],
            stdout=gs_log,
            stderr=gs_log,
            cwd="/home/cuava/Ops"
        )
    ...
    # Wait for 3 minutes and then check if process_2 is still running
    time.sleep(480)  # Wait for 7 minutes
    if process_gsautorun.poll() is None:  # Check if process_2 is still running
        # os.kill(pid_2, 15)  # Terminate gs-autorun
        kill_process_and_children(pid_2)  # Terminate gs-autorun and its children
    ...
    for i in range(1):
        ...
        time.sleep(90)  # Wait for 1.5 minute
    ...
    # time.sleep(360)  # Wait for 10 minutes
